    ...
    while(1){
        ...
        for(;;){
            ...
            * */
            ...
            //解析报文
            parse_sock_rx(accsocfd, recv_buf, recv_len);
        }
...
{
	tc_spi_send(spi_tx_buf, spi_tx_buf_len);
}

static int server_spi_transfer(char* spi_tx_buf, int spi_tx_buf_len,
	char* spi_rx_buf, int spi_rx_buf_len)
{
	return tc_spi_transfer(spi_tx_buf, spi_tx_buf_len, spi_rx_buf, spi_rx_buf_len);
}
...
int server_transfer(int sock_fd,char* sock_data, int sock_data_len)
{
    ...
    server_tx_buf_len = server_spi_transfer((char *)server_sdata->payload, sizeof( server_sdata->payload),
...
int server_send_interrupt(void)
{
    ...
    int_sdata.crc_res = 0x00;

    if(-1 == send(accsocfd,&int_sdata,sizeof(sock_data_st),0)){
